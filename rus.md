...
версию. Но для нашего сайта мы хотели **взять лучшее от обоих подходов**: 
получить преимущества и отзывчивого дизайна, и отдельной версии для мобильных. 
...
прорабатываю типографику проекта, стиль и общее визуальное 
...
В отзывчивом дизайне подход «[сначала мобильные][3]» не является чем-то новым, и уже 
некоторое время он считается одним из лучших. Согласно этому подходу, начинать 
разработку следует с создания макета мобильной версии сайта, и затем с помощью 
медиавыражений адаптировать его к экранам больших размеров. 
...
использование акварельной текстуры в качестве фонового изображения. 
Это довольно большая картинка, и она должна быть включена для показа только
на больших экранах (шириной 660 пикселей и больше). Поскольку наш CSS начинается со 
стилей для мобильных устройств, это изображение никогда не попадёт на устройство 
с маленьким экраном, ведь медиавыражение, которое его загружает, активируется 
...
экранов с шириной не менее 660 пикселей, добавляет ряд других изменений в макет 
...
устройства и позволили дизайну диктовать контрольные точки медиавыражений. 
...
использовали ключевые точки, ориентируясь на размеры конкретных экранов. 
...
В дальнейшем, когда размер экрана увеличивается и выходит за пределы в 660 
...
использованием пикселей, и это очередной пример того, как мы совершенствуем наше 
...
Продолжая следовать принципу «сначала мобильные», мы стилизировали ссылку 
...
`servicesTab`, и для каждого раздела сайта для тега `body` прописан класс. Для 
...
использовать, чтобы отобразить это подменю, когда мы находимся в соответствующем разделе.
...
навигация действительно максимально отзывчива и к требованиям дизайна, 
...
Определение устройства сделало возможными другие незначительные 
...
внесения небольших изменений в объём загружаемого контента. 
...
объясняется, как это проще сделать, добавив маленький скрипт в системный файл 
...
изображениями. В отличие от иллюстраций к анонсам на главной странице, изображения 
в статьях тесно связаны с контентом, так как помогают донести смысл статьи
или акцентировать его. В то время, как изображения очень важны, они также имеют довольно
...
Для доски объявлений на главной странице мы использовали аналогичный подход. 
Для продвижения важных новостей о будущих событиях, 
новых услугах и просто объявлений используются большие баннеры с текстом и изображениями. 
**Доска объявлений — это еще одно полезное дополнение**, которое можно оставить
...
содержимое в действие. Мы получили эффективный инструмент, который 
позволяет выводить разную разметку на разные устройства и таким образом
...
существенно уменьшить размер главной страницы — с 738,3 КБ для десктопной версии
до 174,4 КБ для мобильных устройств.
